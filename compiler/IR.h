...
public:
  ...
  std::string test_var_name; /** < when generating IR code for an if(expr) or
                    while(expr) etc, store here the name of the variable that
...
class CFG {
public:
  CFG(DefFunction *ast, AssemblyLangage assemblyLangage = x86,
      bool debug = false, bool symbol = false);
  ~CFG();
  ...
  std::string symbol_to_asm(
      const std::string
          &reg); /**< helper method: inputs a IR reg or input variable, returns
  ...
  bool symbolIsRegRet(const std::string &symbol) const;
